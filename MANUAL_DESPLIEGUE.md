...
## Ejecutar con Docker (local o servidor)

Esta guía rápida levanta la app y PostgreSQL con Docker Compose.

### Puertos usados por defecto en este proyecto

- App web: `8080`
- PostgreSQL: `5433`

> Se evita usar `80`, `8081`, `3306` y `6379`.
...
2. Crea los archivos de secretos (no se guardan en `.env`):

```bash
mkdir -p secrets
printf '%s' 'tu_secret_key_larga' > secrets/secret_key
printf '%s' 'tu_usuario_smtp' > secrets/mail_username
printf '%s' 'tu_password_smtp' > secrets/mail_password
printf '%s' '' > secrets/stripe_secret_key
chmod 600 secrets/secret_key secrets/mail_username secrets/mail_password secrets/stripe_secret_key
```

3. Edita `.env` para configuración no sensible (`DATABASE_URL`, `MAIL_SERVER`, `MAIL_PORT`, etc.).
...
8. Parar y borrar también los datos de PostgreSQL:

```bash
docker compose down -v
```

### Configurar backup y S3 desde Admin

1. Entra en **Admin → Backups** (`/admin/backups`).
2. En la configuración de backups define:
   - intervalo en horas,
   - retención de días,
   - ruta local (recomendado `/app/backups`).
3. Activa **Subir backup a S3** y completa bucket/región/prefijo.
4. Guarda Access Key y Secret Key desde el mismo formulario (se guardan cifradas en BD).
5. Usa **Ejecutar backup ahora** para hacer una prueba manual.

> El servicio `backup` de Docker Compose revisa la configuración cada minuto y ejecuta copias cuando corresponde.
...
> Para SMTP en puerto `465`, usa normalmente `MAIL_USE_SSL=true` y `MAIL_USE_TLS=false`.
